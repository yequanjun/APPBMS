$(function () {
    ...
    $.ajax({
            type: "GET",//请求类型
        < < < < < < < HEAD
    url:"/doAppInfo/queryAllStatus",//请求的url
======
    =
        url
:
    "/doDeveloper/dictionary.json",//请求的url
>>>>>>>
    origin / master
...
,
    ...
    $.ajax({
        type: "GET",//请求类型
        url: "/doDeveloper/appLevel1.json",//请求的url
        dataType: "json",//ajax接口（请求url）返回的数据类型
        success: function (data) {//data：返回数据（json对象）
            $("#categoryLevel1").html("");
            var options = "<option value=\"\">--请选择--</option>";
    ...
    $("#APKName").bind("blur", function () {
        ...
        $.ajax({
            type: "GET",//请求类型
            url: "/doDeveloper/confirmAPKNameExitOrNot",//请求的url
